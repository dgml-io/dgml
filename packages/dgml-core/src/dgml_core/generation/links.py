...
class PlanLosses:
    ...
    - ``nested`` — every object is on the subject's own root-to-leaf path, so
      ...
      is not counted here.
    - ``merged`` — folded into an earlier link on the same subject that states
      the same relationship (see :func:`_resolve_plan`). Nothing is lost: the
      objects join that link's ``dg:href``. Counted because the entry does not
      become a link of its own.
    - ``displaced`` — ``dg:itemprop``/``dg:href`` are attributes *on the subject
      element*, so a second link on one subject with a DIFFERENT relationship
      overwrites the first and only the last survives. That one is a limit of
      the format rather than a decision made here, and it is the residue left
      once merging has taken what it can.
    ...
    Every entry either becomes a link of its own or lands in one of these, so
    ...
    @property
    def total(self) -> int:
        return self.nested + self.merged + self.displaced + self.off_tree


def _merged_value(held: str, incoming: str) -> str | None:
    """The one ``dg:value`` two merging entries can share, or ``None`` if they
    cannot: a subject carries a single ``dg:value``, so two different non-empty
    ones are a real conflict and the entries must not be folded together."""
    if held == incoming or not incoming:
        return held
    if not held:
        return incoming
    return None
...
def _resolve_plan(
    ...
    off_tree = nested = merged = displaced = 0
    for item in plan:
        ...
        if previous is None:
            ...
            continue
        # A subject can carry only one dg:itemprop/dg:href pair. But dg:href
        # already holds a LIST of objects — that is how a lesser-of formula
        # names its operands — so two entries stating the SAME relationship
        # about the same subject are one link with two objects, not a conflict.
        # Folding them keeps both; only a genuine disagreement about the
        # relationship still costs a link.
        _, held_objects, held_predicate, held_value = resolved[previous]
        value = _merged_value(held_value, entry[3])
        if held_predicate == entry[2] and value is not None:
            objects = held_objects + [o for o in keep if o not in held_objects]
            resolved[previous] = (subject_idx, objects, held_predicate, value)
            merged += 1
        else:
            resolved[previous] = entry  # different relationship: the last wins
            displaced += 1
    return resolved, PlanLosses(
        nested=nested, merged=merged, displaced=displaced, off_tree=off_tree
    )
