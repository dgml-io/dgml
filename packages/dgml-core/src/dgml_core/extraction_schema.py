...
def vocabulary_to_json_schema(vocab: Vocabulary) -> dict[str, Any]:
    ...
    Carries ``description``/``example`` onto each property so the RNC ⇄ JSON ⇄
    RNC round-trip is lossless. The extraction engine ignores those extra keys
    (it only walks for grounded_field $refs and properties).
    """
    return {
...
class _RawDef:
    ...
    example: str | None
    prompt: str | None
    body_lines: list[str]  # content-model lines
