...
def test_every_plan_entry_either_lands_or_is_accounted_for() -> None:
    ...
    plan = [
        _one(2, [3]),  # lands
        _one(2, [5]),  # same subject: displaces the one above
    ...
    assert (losses.overwritten, losses.nested, losses.off_tree) == (1, 1, 2)

    # The XML agrees: one link, the last one written on that subject.
    linked, applied = apply_plan(_TREE, plan)
    assert len(applied) == 1 and applied[0].objects == ["cap"]
