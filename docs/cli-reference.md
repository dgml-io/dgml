...
| `--no-semlink-cache` | off | Always call the model for the semantic-link pass. The pass is a pure function of its inputs, so by default it is content-addressed on (grounded XML, labeling model, link prompts) under the docset's `cache/semlinks/` prefix: a repeat run replays the cached result — the same bytes and the same `links` count the model call would have produced — instead of paying for it again. This flag forces a fresh call — use it when something the key cannot see has changed, such as a provider-side model update behind a stable model id. |
